  ...
  <div>
    <a-card>
      ...
      <space-between>
        ...
        <a-button @click="show = true" v-only-admin>添加出库单</a-button>
      </space-between>
      <a-divider></a-divider>
      <a-table :columns="columns" :data-source="list" :pagination="false" bordered>
        ...
        <template #customer="{ record }">
          {{ getCustomerNameById(record.customer) }}
        </template>
        <template #actions="record">
          <a href="javascript:;" @click="updateCount(record)" v-show="record.record.status === 0">出库</a>
          <span v-show="record.record.status === 1">已出库</span>
          &nbsp;
    ...
    <update v-model:show="showUpdateModal" :outCount="curEditOutCount" @update="updateCurOutCount"></update>
  </div>
</template>
